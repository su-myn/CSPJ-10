...
@app.route("/set-admin")
def set_admin():
    """Set first user as admin - no authentication required (temporary route for admin recovery)"""
    db = get_db()
    ...
    if admin_count == 0:
        ...
        if first_user:
            ...
            return f"""<html><body style="font-family: Arial, sans-serif; padding: 20px; text-align: center;">
                ...
                <p style="color: #666; font-size: 12px; margin-top: 30px;">⚠️ Remember to remove this route after fixing admin access!</p>
            </body></html>"""
        else:
            db.close()
